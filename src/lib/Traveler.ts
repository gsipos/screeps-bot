// source: https://github.com/bonzaiferroni/Traveler
  ...
  public static travelTo(
    ...
    if (!travelData.path) {
      ...
      if (state.cpu > REPORT_CPU_THRESHOLD) {
        ...
        console.log(
          `TRAVELER: heavy cpu use: ${creep.name}, cpu: ${state.cpu} origin: ${
            creep.pos
          }, dest: ${destination}`
        );
      }
  ...
  public static serializePath(
    ...
    for (let position of path) {
      if (position.roomName === lastPosition.roomName) {
        new RoomVisual(position.roomName).line(
          position.x,
          position.y,
          lastPosition.x,
          lastPosition.y,
